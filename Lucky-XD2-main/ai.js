...
malvin({
    ...
    category: "ai",
    react: "🤖",
    use: ".Kaleem <your query>",
    filename: __filename,
}, async (conn, mek, m, { from, args, q, reply }) => {
    try {
        // Vérification de l'entrée utilisateur
        if (!q) return reply("⚠️ Please provide a query for Kaleem Ai.\n\nExample:\n.Kaleem What is AI?");
        ...
        if (!response || !response.data || !response.data.result) {
            return reply("❌ No response received from the Kaleem API. Please try again later.");
        }
        ...
        const formattedInfo = `🤖 *Kaleem's Response:*\n\n${gptResponse}`;
